...
// @attribution	changes [d:09.27.12][u:<ul><li>Fix for the new permissions as I keep working to comply with GM Standards</li></ul>]
// ==/UserScript==
...
function getRealURL(urlID) {
	...
	plusBox.innerHTML = '<img title="Click to fetch full URL" style="padding:2px;border:none" src="'+b3+'" alt="[+]">';
	GM_xmlhttpRequest({
		method:"HEAD",
		url:theURL,
		...
		onload:function(content){
			...
			minusBox.innerHTML = '<img title="Click to collapse back to shortened URL" style="padding:2px;border:none" src="'+b2+'" alt="[-]">';
			minusBox.href="javascript:void(0);";
			minusBox.id = 'bsl'+urlID.substr(2);
			minusBox.addEventListener("click",function(){
				this.innerHTML = '<img title="Click to fetch full URL" style="padding:2px;border:none" src="'+b1+'" alt="[+]">';
		...
		if (newLinks.length > 0)
		{
			for (var i = 0; i < newLinks.length; i++) {
				if (newLinks[i].className.match(/\bsmgm_usTitle\b/) == null && newLinks[i].innerHTML.indexOf('<img') < 0)
				{
					...
					plusBox.innerHTML = '<img title="Click to fetch full URL" style="padding:2px;border:none" src="'+b1+'" alt="[+]">';
